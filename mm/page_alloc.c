...
int page_alloc_order[MAX_ORDER];
#endif

#if defined(CONFIG_SYSCTL) && defined(CONFIG_PROC_FS)
#ifdef CONFIG_DEBUG_PAGE_ALLOC_ORDER
static int proc_page_alloc_order_write(struct ctl_table *table, int write, 
		void __user *buffer, size_t *lenp, loff_t *ppos)
{
	/* return proc_dointvec(table, write, buffer, lenp, ppos); */
}

static int proc_page_alloc_order_read(struct ctl_table *table, int write, 
		void __user *buffer, size_t *lenp, loff_t *ppos)
{
	return proc_dointvec(table, write, buffer, lenp, ppos);
}

int proc_page_alloc_order(struct ctl_table *table, int write, void __user *buffer,
		     size_t *lenp, loff_t *ppos)
{	
	if (write)
		return proc_page_alloc_order_write(table, write, buffer, lenp, ppos);
	else	
		return proc_page_alloc_order_read(table, write, buffer, lenp, ppos);
}
...
#ifdef CONFIG_SORT_FREELIST
static void list_add_page_sorted(struct page *page_to_add, struct list_head *head)
{	
	
	struct list_head *curr, *next, *prev;
	unsigned long pfn, pfn2;
	unsigned long pfn_to_add = page_to_pfn(page_to_add);
	...
	list_for_each(curr, head) {
		...
		if (pfn_to_add < pfn && prev == head) {
			list_add(&page_to_add->lru, prev);
			return;
		}

		if (pfn_to_add > pfn && next == head) {
			list_add(&page_to_add->lru, curr);
			return;
		}

		pfn2 = page_to_pfn(list_entry(next, struct page, lru));

		if (pfn_to_add > pfn && pfn_to_add < pfn2) {
			list_add(&page_to_add->lru, curr);
			return;
		}
	}
...
#else
	list_add_page_sorted(page, &zone->free_area[order].free_list[migratetype]);
#endif
...
#else
		list_add_page_sorted(&page[size], &area->free_list[migratetype]);
#endif
		area->nr_free++;
...
#else
		list_add_page_sorted(page, list);
#endif
...
#else
	struct page *page =
		pfn_to_page((addr - (unsigned long)total_ram) >> PAGE_SHIFT);
	if (addr != 0) {
		VM_BUG_ON(!pfn_valid(page_to_pfn(page)));
		__free_pages(page, order);
	}
#endif
}
				...
				unsigned long zonesize)
{
	...
	printk(KERN_INFO "usemapsize:%lu, pgdata:%p\n", usemapsize, pgdat);
	if (usemapsize)
...
static void __init_refok alloc_node_mem_map(struct pglist_data *pgdat)
{
	...
	if (!pgdat->node_mem_map) {
		...
		if (!map) {
			...
			printk(KERN_INFO "I am %s: %p, size:%lu\n", __func__, map, sizeof(struct page));
		}
